...
def evaluate_board(boardNew,whosMove):
    points = 0
    if(board.checkmate(boardNew, whosMove)):
        points = 100
        return points
    changedWhosMove = changeWhosMove(whosMove)
    if(board.checkmate(boardNew, changedWhosMove)):
        points = -100
        return points
    for row in boardNew:
        for piece in row:
...
def minimax(boardNew, depth, alpha, beta, maximizing_player, EnPassantAI,whosMove):
    ...
    if depth == 0 or board.checkmate(boardNew, "b") or board.checkmate(boardNew, "w"):
        return None, evaluate_board(boardNew,whosMoveAI)
